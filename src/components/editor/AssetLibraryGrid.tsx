...
export function AssetLibraryGrid({
  ...
  if (assets.length === 0) {
    return <div className="rounded-md border border-dashed border-border-strong p-6 text-sm text-muted">No assets yet.</div>;
  }

  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
      {assets.map((asset) => (
        <div key={asset.id} className="overflow-hidden rounded-md border border-border-strong bg-panel-2">
